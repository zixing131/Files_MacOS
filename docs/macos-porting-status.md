...
| Tab management and closed-tab recovery | Passed full-state runtime round trips | Tabs can be dragged to reorder, duplicated beside the source with Command/Control-Shift-K, closed to the left/right or closed except for the selected tab through localized context and native File menus. Up to 20 closed tabs reopen with Command/Control-Shift-T; primary/secondary paths, active pane, split ratio, grid/details mode and sorting restore together in nearest-tab order, and the reordered tab sequence is persisted with the workspace |
...
- Tabs drag to reorder and can be duplicated beside their source; localized tab context and native File menus close left/right/other tabs or reopen the latest of 20 closed tabs with its complete dual-pane state. Command/Control-Shift-K duplicates and Command/Control-Shift-T recovers.
