...
| Tab management and closed-tab recovery | Passed full-state runtime round trips | Tabs can be dragged to reorder or outside the strip to create a new native window, duplicated beside the source with Command/Control-Shift-K, switched by bounded wheel scrolling or wrapping Control-Tab/Control-Shift-Tab, and closed to the left/right/except-selected through localized context and native menus. The wheel direction is configurable, up to 20 closed tabs restore complete dual-pane state, and reordered/transferred window workspaces persist without treating a move as a close |
