...
| Settings and workspace persistence | Passed isolated and runtime restore tests | Versioned atomic JSON preserves preferences, language, recent paths, search history/saved searches, every open window and its tabs, pane paths, active window/tab/pane, split ratios, view, sort state, sidebar visibility/width/collapsed sections, preview visibility and access bookmarks; schema v10 caps restored windows/tabs, migrates older sidebar state, validates language and section IDs, clamps restored values, normalizes duplicate grants and restricts the settings file to user read/write |
...
| Sidebar navigation fidelity | Passed runtime round trips | The longest matching sidebar path follows the active pane (for example Downloads takes precedence over Home), section headers expose vector disclosure indicators, Favorites/Recent/Libraries/Network/Drives collapse independently, and section state persists through schema v10 |
...
| Recent locations | Passed isolated persistence/runtime checks | Successful navigation records up to eight non-built-in folders, the collapsible sidebar group avoids duplicates with libraries and mounted volumes, a visible footer action clears history, and schema v10 normalizes invalid section state and duplicate paths |
| Native macOS menus, windows and shortcuts | Passed build/runtime checks | Localized application, File, Edit, View, Go and Window menus route through the active window with live validation snapshots; New Window and Close Window use conventional Command shortcuts, up to eight windows restore their independent tab/dual-pane workspaces and prior active-window ordering across launches, concurrent preference saves merge without clobbering newer values, and 16 Command/Command-Shift/Command-Option accelerators coexist with the portable Control bindings |
...
- Independent macOS windows through File > New Window / Command-N, with Command-Shift-W close behavior, active-window menu routing, retained window lifetimes and schema-v10 restoration of every window's tab/dual-pane workspace and active-window index.
