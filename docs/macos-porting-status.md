...
| Finder metadata editing | Passed native/runtime round trips | The single-item properties dialog edits validated octal Unix modes and comma-separated Finder tags; native tag writes and readback passed on a temporary file, and a failed tag write rolls the permission change back |
...
- Editable single-item Finder tags and validated Unix permissions in the localized properties dialog, with transactional rollback if native metadata saving fails.
