...
| Finder metadata and security properties | Passed native/runtime round trips | The polished, localized single-item properties dialog groups general and sharing details, displays owner/group IDs and the POSIX ACL, and edits validated Unix modes, Finder tags, hidden and locked flags; native readback, immediate hidden filtering and transactional rollback passed on a temporary file |
...
- Single- and multi-selection properties with recursive size/count, timestamps, Unix modes and symbolic-link targets; single-item security details include owner/group IDs and POSIX ACL entries.
- Editable single-item Finder tags, validated Unix permissions, and Finder-compatible hidden/locked flags in the localized properties dialog, with transactional rollback if any metadata save fails.
