...
| NTFS ACL/安全属性 | 已实现 POSIX mode 编辑、ACL 与 owner/group 展示，以及隐藏/锁定标记编辑；owner/group 与 ACL 修改仍待补齐 | 部分实现 |
...
- macOS 菜单栏、标准快捷键、服务菜单、窗口恢复和原生对话框。
- iCloud Drive、第三方 File Provider、SMB/NFS、网络断开和占位文件状态。
- 补齐 POSIX ACL 与 owner/group 编辑，并继续验证 Finder tags、隐藏/锁定标记与扩展属性在真实 File Provider 上的行为。
