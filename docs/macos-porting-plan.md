...
- macOS 菜单栏、标准快捷键、服务菜单、窗口恢复和原生对话框。独立多窗口、活动窗口菜单路由、并发安全持久化、所有窗口工作区及 AppKit 尺寸/位置/最大化状态的跨启动恢复已完成，并会在显示器变化后约束回可见区域；透明全内容标题栏已将 Files 标签栏与原生红黄绿按钮合并到同一顶部区域。标签页已补齐响应式等宽布局、选中/悬停状态、悬停关闭按钮、已知目录中英文标题、Control-1…9、拖动排序/拖出新窗口、完整状态复制/移动、可反转滚轮切换、Control-Tab、关闭左侧/右侧/其他标签页及 Command-Shift-T 恢复，并接入中英文右键和原生菜单。
- iCloud Drive、第三方 File Provider、SMB/NFS、网络断开和占位文件状态。
- macOS 文件包按 Finder 语义打开和搜索：常见应用、文稿、插件与开发者 bundle 已通过受管扩展集识别，保留真实目录操作语义，并在界面中作为可启动项目展示；后续可用 Foundation URL resource values 补齐第三方动态注册的 package 类型。
- 继续打磨共享与权限编辑体验，并验证 Finder tags、ACL、身份及隐藏/锁定标记在真实 File Provider 上的行为。
- FSEvents 大目录监控、缩略图分级缓存和操作队列恢复。
- VoiceOver、完整键盘操作、高对比度/Reduce Motion/Reduce Transparency。主界面已加入 F6/Shift-F6 区域循环，侧边栏/双窗格分隔线的方向键与 Home/End 调整、系统焦点描边，以及标签、侧边栏、路径/搜索、工具栏和双窗格文件项目的中英文辅助名称；文件项目会预计算类型、隐藏状态、大小、修改日期和搜索位置，分组标题按 Return/Space 才切换并播报展开状态，详细信息列标题同步播报排序字段/方向，状态栏和预览变化通过 AppKit 原生通知播报。AppKit 桥也会在窗口创建/应用重新激活时应用增强对比度、减少透明度和减少动态效果。仍需真实 VoiceOver 顺序/播报及三项系统偏好的人工切换验收。
