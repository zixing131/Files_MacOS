...
| Sidebar navigation fidelity | Passed runtime round trips | The longest matching sidebar path follows the active pane (for example Downloads takes precedence over Home), section headers expose vector disclosure indicators, Favorites/Recent/Libraries/Network/Drives collapse independently, and section state persists through schema v9 |
